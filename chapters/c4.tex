...
\begin{lstlisting}[language=Python, title=Extracting 3D Position-Invariant Features]
landmark_data = []
...
\begin{lstlisting}[language=Python, title=Training a Random Forest Model]
import pandas as pd
...
With a trained model capable of recognizing gestures, we can import the \texttt{pyautogui} library to bridge the gap between Python and the operating system. 

To map the physical movement of the hand to the monitor, we track the tip of the index finger (Landmark 8). Instead of a direct 1:1 mapping, which would force the user to stretch their arm uncomfortably far to reach the screen edges, we use \texttt{numpy.interp} to create an "active bounding box." By mapping the middle 70\% of the camera's field of view (0.15 to 0.85) to the full resolution of the screen, the cursor becomes highly responsive to small, comfortable wrist movements. 

When the ML model classifies the hand structure as a closed fist ("0"), the script triggers a left-click event.

\begin{lstlisting}[language=Python, title=HCI Mouse Control Integration with Interpolation]
import pyautogui
import numpy as np
import time

# Get the monitor's true resolution automatically
screen_w, screen_h = pyautogui.size()

# ... [Inside the MediaPipe detection loop] ...

# 1. Mouse mapping logic using the Index Finger
x_pure = hand_landmarks.landmark[8].x
y_pure = hand_landmarks.landmark[8].y

# Interpolate the camera percentage (15% to 85%) to screen pixels
target_x = np.interp(x_pure, [0.15, 0.85], [0, screen_w])
target_y = np.interp(y_pure, [0.15, 0.85], [0, screen_h])
...
\noindent\fbox{%
    \parbox{\textwidth}{%
        \begin{center}\textbf{\LARGE BEFORE MOVING ON}\end{center}
        Before moving on to the exercises, please note: If you encountered hardware issues, strict camera permissions, or ran out of time during the data collection phases of this tutorial, please refer to \hyperref[sec:annex]{\underline{\textbf{Annex A}}} at the end of this document. It contains a direct link to download a pre-compiled dataset and a pre-trained AI model, guaranteeing you can complete the upcoming challenges.
    }%
}
\vspace{0.4cm}